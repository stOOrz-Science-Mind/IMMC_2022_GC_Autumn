...
long long
GetLineCircleIntersection(const Point &A, const Point &B, const Circle &C, vector <Point> &sol)//ֱ�ߺ�Բ�Ľ�������
{
...
bool is_out(Car vehicle) {
    ...
    bool c3 = (vehicle.center.x >= SEP && vehicle.center.x <= RIGHT &&
               vehicle.center.y < 0.49 * vehicle.center.x - 73.8 && Dot(vehicle.Direction, TO_RIGHT) > 0);
...
turn rand_turn(long long crossroad_number,
               long long status) { // rand-status=0 ��ͷ rand-status=1 ��ת rand-status=2 ֱ�� rand-status=3 ��ת
//	cout<<"*********************************************************";
...
double max_upspeed(double a, double s, double maxv, long long cross) {
    ...
    */
    ...
    double red_time = (crosses[cross].red + crosses[cross].green) -
                      mod(dealt_time + crosses[cross].initstatus, crosses[cross].red + crosses[cross].green);
...
//		cout<<cs[k].center.x<<","<<cs[k].center.y<<")\n ``````````````````````````````` \n";
    ...
    for (long long i = k; i <= k; i++) {
        printf("%d,%lld,%lf,%lf,%lld,%lf,%d,\n", (int) (dealt_time * 2 + 1), i, cs[i].center.x, cs[i].center.y,
               cs[i].rd, Length(cs[i].Direction), cs[k].next_turn);
    }
...
//				if (sites[crsrd_num] == sites[roads[cs[k].rd].start]) {
        cs[k].Direction = (sites[roads[cs[k].rd].end] - sites[roads[cs[k].rd].start]) * (Length(cs[k].Direction) /
                                                                                         point_dist(
...
//				}
    ...
    if (car_turning[k] > 0) {
        ...
        if (accelerate_time[k] > 0) {
            cs[k].Direction =
                    cs[k].Direction * (Length(cs[k].Direction) + 0.5 * max_acceleration) / Length(cs[k].Direction);
...
//					cout<<"="; 
                if (point_dist(cs[k].center, sites[crsrd_num]) <= Length(cs[k].Direction) * 0.5) cs[k].next_turn = 1;
            } else {
                if (point_dist(cs[k].center, sites[crsrd_num]) <=
                    min_decelarate_dis(Length(cs[k].Direction), max_acceleration)) { // ɲ��
//						cout<<"&";
...
//	cout<<is_out(Car(-6.859625, 47.807942, 12, 13.888889, 0))
    ...
    freopen("dat.csv", "w", stdout);
    cout << "t,#,x,y,road,speed,turn,\n";
...
//	cout<<"\n----------\n";
    ...
    for (long long i = 1; i < 18; i++) {
        if (crosses[i].roadnumber == 1)
            crosses[i].outrd[1] = crosses[i].outrd[2] = crosses[i].outrd[3] = crosses[i].outrd[0];
    ...
    for (long long i = 1; i <= 17; i++) {
        crosses[i].green = 100 * (roads[crosses[i].outrd[0]].width) /
                           (roads[crosses[i].outrd[0]].width + roads[crosses[i].outrd[1]].width);
