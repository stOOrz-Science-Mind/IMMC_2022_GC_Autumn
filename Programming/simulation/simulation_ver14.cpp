...
void update(long long k) { // 0.5second
	...
	for (long long i = k; i <= k; i++) {
		printf("%lld�ų�: x=%lf, y=%lf, on road number%lld, speed=%lf\n", i, cs[i].center.x, cs[i].center.y, cs[i].rd, Length(cs[i].Direction));
	}
	posx[i][dealt_time * 2 + 1] = cs[i].center[x];
	posy[i][dealt_time * 2 + 1] = cs[i].center[y];
	speed[i][dealt_time * 2 + 1] = Length(cs[i].Direction);
//	cout << "---------------------------------\n";
