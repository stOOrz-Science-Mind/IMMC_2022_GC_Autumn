...
carData = pd.read_csv("dat.csv")
carData = carData.drop(columns = ["Unnamed: 7"])
carData = carData.drop(columns = ["speed", "turn"])
carData = carData.drop(columns = ["road"])

tmax = 41
cars = 10
timeCar = [None for i in range(1, tmax + 2)]
for i in range(1, tmax + 1):
    tmp = carData.loc[carData['t'] == i]
    tmp = tmp.reset_index()
    tmp = tmp.drop(columns = ['t', 'index'])
    timeCar[i] = tmp

cbasic = ["red", "orange", "yellow", "green", "violet", "blue", "purple", "brown"]
cs = 8

cCar = cbasic * (int(cars / cs)) + cbasic[0 : cars % cs]
...
for i in range(1, tmax + 1):
    ...
    gpd.GeoSeries(multiStreetsArea).plot(ax = ax, color = "grey", alpha = 0.25)

    gpd.GeoSeries(multiLidarArea).plot(ax = ax, color = "red", alpha = 0.01)

    gpd.GeoSeries(multiWifiArea).plot(ax = ax, color = "yellow", alpha = 0.01)

    ax.scatter(lampx, lampy, s = 10, alpha = 0.5)

    ax.scatter(timeCar[i]["x"], timeCar[i]["y"], s = 40, c = cCar)
...
gpd.GeoSeries(multiLidarArea).plot(ax = ax, color = "red", alpha = 0.03)

gpd.GeoSeries(multiWifiArea).plot(ax = ax, color = "yellow", alpha = 0.03)

ax.scatter(lampx, lampy, s = 10, alpha = 0.5)

for i in range(1, tmax + 1):
    ax.scatter(timeCar[i]["x"], timeCar[i]["y"], s = (i + 5) / (tmax + 5) * 40, c = cCar, alpha = (i + 5) / (tmax + 5))
